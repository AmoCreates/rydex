...
const PartnerEarning = () => {
	...
	const delta =
		today && yesterday
			? (today.earnings ?? 0) - (yesterday.earnings ?? 0)
			: 0;
	const deltaPositive = delta >= 0;
	const percentage = yesterday
		? Math.abs(Math.round((delta / (yesterday.earnings ?? 1)) * 100))
		: 0;
			...
			<div className="flex items-start justify-between mb-6 flex-wrap gap-4">
				...
				<div className="text-right">
					...
					>
						...
						<span>{percentage}% vs yesterday</span>
					</div>
				</div>
			</div>
