...
const Page = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setIsLoading(false);
    // router.push('/partner/onboarding/next-step');
  };
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex items-center justify-center px-4 py-10">
			<motion.div
				...
				className="relative w-full max-w-150 bg-white rounded-2xl border border-gray-100 shadow-[0_25px_70px_rgba(0,0,0,0.08)] overflow-hidden"
			>
        {/* Back Button */}
        <button 
          onClick={() => router.back()}
          className="absolute left-8 top-8 z-10 p-3 bg-gray-50 hover:bg-gray-100 rounded-2xl transition-all group"
        >
        ...
        <div className="p-6 sm:px-10 sm:py-8">
          <div className="mb-8 text-center">
            <h1 className="text-3xl font-black text-gray-900 tracking-tight">Pricing & Vehicle Image</h1>
            <p className="text-gray-500 mt-2 font-medium">Configure your vehicle service details</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-8">
                ...
                {imagePreview ? (
                  <>
                    ...
                    <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                      <span className="text-white font-bold bg-white/20 backdrop-blur-md px-4 py-2 rounded-xl">Change Photo</span>
                    </div>
                  </>
                ) : (
                  <div className="text-center p-6">
                    ...
                    <p className="text-xs text-gray-400 mt-1 font-medium">Clear side-view photo required</p>
                  </div>
                )}
              </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="space-y-4">
                ...
                <button
                  type="button"
                  onClick={() => setFormData(p => ({ ...p, acAvailable: !p.acAvailable }))}
                  className={`w-full flex items-center justify-between px-5 py-4 rounded-2xl border-2 transition-all ${
                    formData.acAvailable ? 'border-black bg-black text-white' : 'border-gray-100 bg-gray-50 text-gray-600'
                  }`}
                >
                  <span className="font-bold">Air Conditioning</span>
                  <div className={`w-12 h-6 rounded-full relative ${formData.acAvailable ? 'bg-white/20' : 'bg-gray-200'}`}>
                    <div className={`absolute top-1 w-4 h-4 rounded-full transition-all ${formData.acAvailable ? 'right-1 bg-white' : 'left-1 bg-gray-400'}`} />
                  </div>
                </button>
              </div>
                  ...
                  {['good', 'fair', 'poor'].map((c) => (
                    <button
                      ...
                      className={`flex-1 rounded-xl text-xs font-black uppercase tracking-tighter transition-all cursor-pointer ${
                        formData.condition === c ? 'bg-black text-white shadow-md scale-[1.02]' : 'text-gray-400 hover:text-gray-600'
                      }`}
                    >
                      {c}
                    </button>
            ...
            <div className="pt-6 border-t border-gray-100 space-y-6">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                {[
                  ...
                  { label: 'Wait / Min', key: 'waitingCharge', icon: Clock },
                ].map((item) => (
                  <div key={item.key} className="px-5 py-4 bg-gray-50 rounded-2xl border border-transparent focus-within:border-black focus-within:bg-white transition-all space-y-1">
