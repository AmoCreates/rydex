...
const Page = () => {
	...
	const [documentsData, setDocumentsData] = useState<IPartnerDocs | null>(
		null,
	);
	...
	const handleDecision = async () => {
		...
		setErrMsg("")
		try {
			const {data} = await axios.put(`/api/admin/reviews/partner/${id}/approve`, {
		...
		<div className="relative min-h-screen bg-linear-to-br from-gray-100 to-gray-200">
			...
			<header className="sticky top-0 z-40 backdrop-blur-xl bg-white/70 border-b">
				<div className="max-w-7xl mx-auto px-4 h-16 flex items-center gap-4">
					<button
						className="w-10 h-10 rounded-full cursor-pointer shadow-md border-zinc-50 border flex items-center justify-center hover:bg-gray-100 bg-white transition"
						onClick={() => router.back()}
					>
						<ArrowLeft />
					</button>
